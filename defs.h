#ifndef __ICM_20948_DEFS_H__
namespace ECE4180 {
    const float DEG_TO_RAD = (1/57.2957795);
    const float RAD_TO_DEG = 57.2957795;

    namespace ICM_20948 {
        //Magnetometer Registers
        const unsigned char AK09916_ADDRESS  = 0x0C; 
        const unsigned char WHO_AM_I_AK09916 = 0x01; // (AKA WIA2) should return 0x09
        const unsigned char AK09916_ST1      = 0x10;  // data ready status bit 0
        const unsigned char AK09916_XOUT_L   = 0x11;  // data
        const unsigned char AK09916_XOUT_H   = 0x12;
        const unsigned char AK09916_YOUT_L   = 0x13;
        const unsigned char AK09916_YOUT_H   = 0x14;
        const unsigned char AK09916_ZOUT_L   = 0x15;
        const unsigned char AK09916_ZOUT_H   = 0x16;
        const unsigned char AK09916_ST2      = 0x18;  // Data overflow bit 3 and data read error status bit 2
        const unsigned char AK09916_CNTL     = 0x30;  // Power down (0000), single-measurement (0001), self-test (1000) and Fuse ROM (1111) modes on bits 3:0
        const unsigned char AK09916_CNTL2    = 0x31;  // Normal (0), Reset (1)
        
        // Using the ICM-20948 breakout board, ADO is set to 1
        // Seven-bit device address is 1000100 for ADO = 0 and 1000101 for ADO = 1
        #define ADO 0 
        #if ADO
        const unsigned char ADDRESS = 0x69<<1;  // Device address when ADO = 1
        #else
        const unsigned char ADDRESS = 0x68<<1;  // Device address when ADO = 0
        #endif // AD0
        ...
        */    
        ...
        enum BANK_REGISTERS {
                        // BANK 0
                                 WHO_AM_I =      0x00, //BANK 0: Should return 0xEA
